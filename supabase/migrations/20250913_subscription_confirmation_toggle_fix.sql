-- Fix: use a plain column for occurred_month (immutable requirement workaround)
...
-- occurred_month as a normal column (nullable) populated by app/RPC
alter table if exists public.transactions
  add column if not exists occurred_month date;
...
-- Update RPC to populate subscription_id and occurred_month and avoid duplicate insertions
...
-- Daily scheduler entry point (unchanged behavior)
